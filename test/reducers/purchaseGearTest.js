...
describe('purchaseGear', () => {
	...
	describe('MODDING_DRONE', () => {
		...
		describe('modding a drones stat', () => {
			...
			it('should raise pilot', () => {
				const newState = reducer(state, { type: 'MODDING_DRONE', parameter: { index: 0, category: 'Drones', mod: dronePilot } });

				expect(newState.Drones[0].mods['Pilot Program (Drone)']).to.equal(dronePilot);
				expect(newState.Drones[0].currentCost).to.equal(8200);
		...
		it('should add a mod to a drone that has been modded', () => {
			const newState = reducer(state, { type: 'MODDING_DRONE', parameter: { index: 1, category: 'Drones', mod: geckoGrip } });

			expect(newState.Drones[1].mods['Gecko Grips (Drone)']).to.equal(geckoGrip);
			expect(newState.Drones[1].currentCost).to.equal(44800);
			expect(newState.nuyen).to.equal(3950);
			expect(newState.Drones[1].currentSlot).to.equal(4);

			expect(state.Drones[1].currentCost).to.equal(44200);
			expect(state.Drones[1].currentSlot).to.equal(3);
